...
import { initializeUnblockMusic, startMusicApi } from './server';
...
function initialize(configStore: any) {
  ...
  // 初始化音乐解析 IPC handler（必须在 startMusicApi 之前调用）
  initializeUnblockMusic();
...
if (!isSingleInstance) {
  ...
  app.whenReady().then(() => {
    // 设置应用ID
    electronApp.setAppUserModelId('com.muqi.music');
