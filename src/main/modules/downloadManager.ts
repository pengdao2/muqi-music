...
class DownloadManager {
  ...
  private async downloadTask(task: DownloadTask): Promise<void> {
    ...
    try {
      ...
      // Temp directory
      const tempDir = path.join(os.tmpdir(), 'MuqiMusicTemp');
  ...
  private async finalizeDownload(
    ...
    try {
      ...
      // 跳过默认封面（旧路径或 SVG Data URL）
      const isDefaultCover =
        !picUrl ||
        picUrl === '/images/default_cover.png' ||
        picUrl.startsWith('data:image/svg+xml,');
      if (!isDefaultCover) {
    ...
    const songInfos = (configStore.get('downloadedSongs') || {}) as Record<string, any>;
    const defaultCoverSvg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" fill="#888"><rect width="200" height="200" rx="16" fill="#2a2a2a"/><text x="100" y="110" text-anchor="middle" font-size="72" fill="#555">♪</text></svg>';
    const defaultInfo = {
      name: task.filename,
      ar: [{ name: '本地音乐' }],
      picUrl: 'data:image/svg+xml,' + encodeURIComponent(defaultCoverSvg)
    };
