...
export const getImageLinearBackground = async (imageSrc: string): Promise<IColor> => {
  try {
    ...
    // 背景色提取失败是正常情况（本地音乐可能无封面等），降级为 debug 日志
    console.debug('[linearColor] 背景色提取失败:', (error as Error).message);
    return {
      backgroundColor: '',
      primaryColor: ''
    };
...
const getImagePrimaryColor = (imageSrc: string): Promise<string> => {
  return new Promise((resolve, reject) => {
    // 空 URL 或无效路径直接返回默认色，避免无意义的错误日志
    if (!imageSrc || imageSrc === '' || imageSrc === 'null' || imageSrc === 'undefined') {
      reject(new Error('无效的图片URL'));
      return;
    }

    // 相对路径 /images/... 在 Electron 中不可用，直接拒绝
    if (imageSrc.startsWith('/images/') && !imageSrc.startsWith('file://')) {
      reject(new Error('相对路径资源不可用'));
      return;
    }

    const img = new Image();
    // base64 Data URL 和 file:// 协议不能设置 crossOrigin，否则 canvas 会报安全错误
    if (!imageSrc.startsWith('data:') && !imageSrc.startsWith('file://')) {
      img.crossOrigin = 'Anonymous';
    }
    img.src = imageSrc;

    // 5秒超时（base64 大图可能加载很慢）
